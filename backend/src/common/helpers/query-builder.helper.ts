...
export class QueryBuilderHelper {
  ...
  static applyDateRange(
    ...
    if (startDate) {
      queryBuilder.andWhere(`${alias}.${dateKey} >= :startDate`, { startDate });
    }

    if (endDate) {
      queryBuilder.andWhere(`${alias}.${dateKey} <= :endDate`, { endDate });
    }
  ...
  static applyBooleanFilter(
    ...
    if (value !== undefined) {
      queryBuilder.andWhere(`${alias}.${fieldName} = :${fieldName}`, {
        [fieldName]: value,
      });
    }
    return queryBuilder;
